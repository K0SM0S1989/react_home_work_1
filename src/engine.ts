...
export const firstPrioritiesCalc = (stack: ParsedLineType): ParsedLineType =>
    stack.reduce<ParsedLineType>((result, nextItem) => {
        ...
        if (nextItem === openBracket) {
            let bracketsArray = stack.splice(stack.indexOf(openBracket),
        ...
        if (!isNumber(String(item)) && mathOperatorsPriorities[item] === FIRST) {
            ...
            result = [
                ...result.slice(0, -2),
                mathSingleNumberOperators.hasOwnProperty(item) ? mathSingleNumberOperators[item](Number(prevItem)) :
...
export const
    secondPrioritiesCalc = (stack: ParsedLineType): number =>
