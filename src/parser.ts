...
export const parser = (line: string): ParsedLineType | null => {
  ...
  return stack.reduce<ParsedLineType>((result, item, key) => {
    ...
    const isValidOpenBrackets =
      !isNumber(prevItem) && isNumber(nextItem) && item === openBracket;
    const isValidCloseBrackets = !isNumber(nextItem) && item === closeBracket;

    let isValidOperatorPush;
    if (
      mathOperators.hasOwnProperty(item) &&
      mathSingleNumberOperators.hasOwnProperty(prevItem)
    ) {
      isValidOperatorPush = !isNumber(item);
    } else if (
      (mathSingleNumberOperators.hasOwnProperty(item) && !isNumber(nextItem)) ||
      mathOperators.hasOwnProperty(item)
    ) {
      isValidOperatorPush =
        (isNumber(prevItem) || prevItem === closeBracket) && !isNumber(item);
    }
